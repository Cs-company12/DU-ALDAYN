...
function duaCardHTML(item) {
  ...
  <article class="dua-card" data-dua="${item.id}">
    ...
    <div class="dua-actions">
      ...
      <button class="counter-btn" data-count="${item.id}">
        <span class="counter-num">${count}</span>
        <span class="counter-lbl">${esc(t("track", lang))}</span>
      </button>
    </div>
  </article>`;
}
...
// re-render once voices finish loading, so the play button reflects real TTS availability
if ("speechSynthesis" in window) {
